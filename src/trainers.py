...
# ADJUST IF NOT USING PYTHIA-410M
# Layer to get activations from for mixup inputs, ideally early in the model
INPUT_EMBEDDING_LAYER = 1
assert(INPUT_EMBEDDING_LAYER > 0)
# Layer to get activations from for mixup outputs, ideally the last layer
OUTPUT_EMBEDDING_LAYER = 5
...
def calculate_mixup_inputs_and_outputs(
    input_embeddings: torch.FloatTensor,
    output_embeddings: torch.FloatTensor,
) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
    mixup_coefficient_distribution = torch.distributions.beta.Beta(
        torch.tensor([1.0]), torch.tensor([1.0])
    )
    mixup_coefficients = (
        mixup_coefficient_distribution.sample([input_embeddings.shape[0]])
        .reshape([input_embeddings.shape[0], 1, 1])
        .to(input_embeddings.device)
    )
    permuted_indices = torch.randperm(input_embeddings.shape[0]).to(
        input_embeddings.device
    )
    permuted_input_embeddings = input_embeddings[permuted_indices]
    permuted_output_embeddings = output_embeddings[permuted_indices]
    mixed_input_embeddings = (
        mixup_coefficients * input_embeddings
        + (1 - mixup_coefficients) * permuted_input_embeddings
    )
    mixed_output_embeddings = (
        mixup_coefficients * output_embeddings
        + (1 - mixup_coefficients) * permuted_output_embeddings
    )
    return mixed_input_embeddings, mixed_output_embeddings


def get_activation(activations: Dict[str, torch.Tensor], layer_name: str):
    # Return a hook function that will store the activations of the given layer in the activations dictionary
...
def get_model_output_embeddings_from_input_embeddings(
    model: nn.Module, input_embeddings: torch.FloatTensor
) -> torch.FloatTensor:
    # Given a model and a batch of input embeddings, return the output embeddings from the model.
    # Uses the global OUTPUT_EMBEDDING_LAYER to determine which layer to use.

    # Forward pass through the model layers up to OUTPUT_EMBEDDING_LAYER
    # Assumes a GPTNeoX model architecture
    x = input_embeddings
    for i, layer in enumerate(model.gpt_neox.layers):
        if i >= INPUT_EMBEDDING_LAYER:
            # Generate position_ids from the model input size and pass None as attention_mask
            position_ids = torch.arange(x.shape[1], dtype=torch.long, device=x.device).unsqueeze(0).expand(x.shape[0], -1)
            print(f"x shape: {x.shape}")
            print(f"position_ids shape: {position_ids.shape}")
            x = layer(x, position_ids=position_ids, attention_mask=None)[0]  # Assuming [0] gets the main output of the layer

        if i == OUTPUT_EMBEDDING_LAYER:
            break  # Stop after processing OUTPUT_EMBEDDING_LAYER

    return x


def get_model_input_and_output_embeddings_for_mixup(
    model: nn.Module, batch: Dict[str, torch.Tensor]
) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
    activations = {}

    # Register hooks to capture the activations. Assumes a GPTNeoX model architecture.
    input_hook = model.gpt_neox.layers[
        INPUT_EMBEDDING_LAYER - 1
    ].mlp.dense_4h_to_h.register_forward_hook(get_activation(activations, "input_layer"))
    output_hook = model.gpt_neox.layers[
        OUTPUT_EMBEDDING_LAYER
    ].post_attention_layernorm.register_forward_hook(
        get_activation(activations, "output_layer")
    )
    ...
    return activations["input_layer"], activations["output_layer"]
...
def get_mixup_loss(
    ...
    (
        input_embeddings,
        output_embeddings,
    ) = get_model_input_and_output_embeddings_for_mixup(model, batch)
    (
        mixed_input_embeddings,
        mixed_output_embeddings,
    ) = calculate_mixup_inputs_and_outputs(input_embeddings, output_embeddings)
    mixed_input_embeddings = mixed_input_embeddings.detach()

    predicted_mixed_output_embeddings = (
        get_model_output_embeddings_from_input_embeddings(model, mixed_input_embeddings)
    )
    return torch.mean(
        torch.sum(
            (predicted_mixed_output_embeddings - mixed_output_embeddings) ** 2, dim=-1
        )
    )
...
class BasicTrainer(object):
    ...
    def get_batch_metrics(
        ...
        if self.mixup:
            mixup_loss = get_mixup_loss(self.policy, batch)
            losses += loss_config.mixup_weight * mixup_loss
            metrics[f"loss/{train_test}/mixup"] = mixup_loss.cpu().numpy().tolist()
    ...
    def train(self):
        ...
        for batch in self.train_iterator:
            ...
            if self.config.loss.name == "dpo" and self.batch_counter % 100 == 0:
                dpo_checkpoint_dir = os.path.join(
                    "experiment-checkpoint", f"dpo-step-{self.batch_counter}"
                )
                rank0_print(f"Saving DPO checkpoint at {dpo_checkpoint_dir}...")
                self.save(dpo_checkpoint_dir, mean_eval_metrics)
            if (
                last_log is None
