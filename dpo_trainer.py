...
class ScriptArguments:
    ...
    max_steps: Optional[int] = field(
        default=100, metadata={"help": "max number of training steps"}
    )
    # instrumentation
...
def main():
    ...
    gold_tokenizer = AutoTokenizer.from_pretrained(
        "sileod/deberta-v3-large-tasksource-rlhf-reward-model"
    )
    gold_reward = AutoModelForSequenceClassification.from_pretrained(
        "sileod/deberta-v3-large-tasksource-rlhf-reward-model"
    )
    preference_dataset = load_dataset(
        "tatsu-lab/alpaca_farm", "alpaca_human_preference"
    )["preference"]
    ...
    preference_dataset = preference_dataset.remove_columns(
        ["instruction", "input", "output_1", "output_2", "preference", "raw_preference"]
    )
