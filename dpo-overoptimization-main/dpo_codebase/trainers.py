...
def calculate_mixup_inputs_and_outputs(
    ...
    mixup_coefficients = (
        ...
        .reshape([input_embeddings.shape[0], 1])
...
def get_model_output_embeddings_from_input_embeddings(
    ...
    for i, layer in enumerate(model.gpt_neox.layers):
        if i >= INPUT_EMBEDDING_LAYER:
            x = layer(x)[0]  # Assuming [0] gets the main output of the layer
...
def get_model_input_and_output_embeddings_for_mixup(
    ...
    input_hook = model.gpt_neox.layers[INPUT_EMBEDDING_LAYER].input_layernorm.register_forward_hook(
        get_activation(activations, 'input_layer')
    )
    output_hook = model.gpt_neox.layers[OUTPUT_EMBEDDING_LAYER].post_attention_layernorm.register_forward_hook(
    ...
    def train(self):
        ...
        for batch in self.train_iterator:
            ...
            if self.config.loss.name == "dpo" and self.batch_counter % 100 == 0:
                dpo_checkpoint_dir = os.path.join('experiment-checkpoint', f"dpo-step-{self.batch_counter}")
